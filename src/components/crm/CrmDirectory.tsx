...
const emptyCompany = { name: '', legalName: '', taxId: '', industry: '', website: '', groupName: '', notes: '', status: 'prospect' as NonNullable<Company['status']> };
...
export default function CrmDirectory({ view }: { view: 'companies' | 'contacts' }) {
  ...
  const filteredCompanies = useMemo(() => {
    ...
    return companies.filter((company) => !term || [company.name, company.legalName, company.groupName, company.industry].some((value) => value?.toLocaleLowerCase('es').includes(term)));
  }, [companies, search]);
  ...
  const editCompany = (company: Company) => {
    ...
    setCompanyForm({ name: company.name, legalName: company.legalName ?? '', taxId: company.taxId ?? '', industry: company.industry ?? '', website: company.website ?? '', groupName: company.groupName ?? '', notes: company.notes ?? '', status: company.status ?? 'prospect' });
    setOpen(true);
  };
  ...
  return (
    <div>
      <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
        ...
        <button className="btn-primary gap-2" onClick={() => setOpen(true)}><Plus size={17} /> {view === 'companies' ? 'Nueva empresa' : 'Nuevo contacto'}</button>
      </div>
          ...
          <div className="grid gap-4 bg-late4-paper/40 p-5 md:grid-cols-2 xl:grid-cols-3">
            {filteredCompanies.map((company) => {
              ...
              return <article className="rounded-xl border border-late4-ink/5 bg-white p-5 shadow-sm" key={company.id}>
                <div className="flex items-start justify-between gap-3"><div className="flex gap-3"><div className="grid h-11 w-11 shrink-0 place-items-center rounded-lg bg-late4-teal-soft text-late4-teal"><Building2 size={20} /></div><div><h2 className="font-extrabold">{company.name}</h2><p className="mt-1 text-xs text-late4-slate">{company.industry || company.legalName || 'Sin actividad registrada'}</p></div></div><button className="grid h-9 w-9 place-items-center rounded-lg bg-late4-paper text-late4-slate" onClick={() => editCompany(company)}><Pencil size={15} /></button></div>
