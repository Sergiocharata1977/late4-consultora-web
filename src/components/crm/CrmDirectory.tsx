...
import { Building2, Download, ExternalLink, Eye, LayoutGrid, Link2, List, Mail, MapPin, MapPinned, Pencil, Phone, Plus, Search, Trash2, X } from 'lucide-react';
...
const emptyCompany = { name: '', legalName: '', taxId: '', industry: '', website: '', groupName: '', location: '', parkName: '', publicContact: '', sourceUrl: '', googleMapsUrl: '', organizationType: 'company' as NonNullable<Company['organizationType']>, notes: '', status: 'prospect' as NonNullable<Company['status']> };
...
export default function CrmDirectory({ view }: { view: 'companies' | 'contacts' }) {
  ...
  const filteredCompanies = useMemo(() => {
    ...
    return companies.filter((company) => (cityFilter === 'all' || company.location === cityFilter) && (!term || [company.name, company.legalName, company.groupName, company.industry, company.location, company.parkName, company.publicContact, company.companySize].some((value) => value?.toLocaleLowerCase('es').includes(term))));
  }, [cityFilter, companies, search]);

  const cities = useMemo(() => [...new Set(companies.map((company) => company.location?.trim()).filter((city): city is string => Boolean(city)))].sort((a, b) => a.localeCompare(b, 'es')), [companies]);
  ...
  const mapsUrl = (company: Company) => company.googleMapsUrl || `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent([company.name, company.location, 'Argentina'].filter(Boolean).join(', '))}`;
  ...
  return (
    <div>
      ...
      <section className="mt-7 overflow-hidden rounded-xl border border-late4-ink/5 bg-white shadow-sm">
        <div className="flex flex-col justify-between gap-3 border-b border-late4-ink/5 p-4 md:flex-row md:items-center"><div className="flex flex-1 flex-col gap-2 sm:flex-row"><div className="relative w-full max-w-md"><Search className="absolute left-3 top-3 text-late4-slate" size={17} /><input className="w-full rounded-lg bg-late4-paper py-2.5 pl-10 pr-4 text-sm outline-none focus:ring-1 focus:ring-late4-teal" onChange={(event) => setSearch(event.target.value)} placeholder={view === 'companies' ? 'Buscar empresa, grupo o actividad' : 'Buscar persona, empresa, email o teléfono'} value={search} /></div>{view === 'companies' && <select className="rounded-lg border border-late4-ink/10 bg-white px-3 py-2.5 text-sm font-bold outline-none focus:border-late4-teal" onChange={(event) => setCityFilter(event.target.value)} value={cityFilter}><option value="all">Todas las ciudades</option>{cities.map((city) => <option key={city} value={city}>{city}</option>)}</select>}</div>{view === 'companies' && <div className="flex gap-2">{([['cards', LayoutGrid], ['list', List]] as const).map(([value, Icon]) => <button aria-label={`Vista ${value}`} className={`grid h-10 w-10 place-items-center rounded-lg border ${companyView === value ? 'border-late4-teal bg-late4-teal-soft text-late4-teal' : 'border-late4-ink/10 text-late4-slate'}`} key={value} onClick={() => setCompanyView(value)}><Icon size={17} /></button>)}</div>}</div>
        ...
        {view === 'companies' && companyView === 'cards' ? (
          ...
          <div className="overflow-x-auto"><table className="w-full min-w-[980px] text-left text-sm"><thead className="bg-late4-paper/70 text-[11px] uppercase tracking-wider text-late4-slate"><tr><th className="px-5 py-3">Empresa</th><th className="px-5 py-3">Ciudad</th><th className="px-5 py-3">Parque / distrito</th><th className="px-5 py-3">Actividad</th><th className="px-5 py-3">Contactos</th><th className="px-5 py-3">Estado</th><th className="px-5 py-3 text-right">Acciones</th></tr></thead><tbody className="divide-y divide-late4-ink/5">{filteredCompanies.map((company) => <tr key={company.id}><td className="px-5 py-4"><p className="font-extrabold">{company.name}</p><p className="text-xs text-late4-slate">{company.organizationType === 'industrial_park' ? 'Parque / distrito' : company.legalName || company.taxId || '—'}</p></td><td className="px-5 py-4 text-base font-extrabold text-late4-teal">{company.location || '—'}</td><td className="px-5 py-4 text-late4-slate">{company.parkName || '—'}</td><td className="px-5 py-4 text-late4-slate">{company.industry || '—'}</td><td className="px-5 py-4 font-bold">{people.filter((person) => person.companyIds.includes(company.id)).length}</td><td className="px-5 py-4">{company.status === 'active' ? 'Cliente' : company.status === 'inactive' ? 'Inactiva' : 'Prospecto'}</td><td className="px-5 py-4"><div className="flex justify-end gap-1"><button aria-label={`Ver ficha de ${company.name}`} className="grid h-9 w-9 place-items-center rounded-lg bg-late4-teal-soft text-late4-teal" onClick={() => setSelectedCompany(company)}><Eye size={15} /></button><a aria-label={`Abrir ${company.name} en Google Maps`} className="grid h-9 w-9 place-items-center rounded-lg bg-late4-paper text-late4-slate" href={mapsUrl(company)} rel="noreferrer" target="_blank"><MapPinned size={15} /></a><button aria-label={`Editar ${company.name}`} className="grid h-9 w-9 place-items-center rounded-lg bg-late4-paper text-late4-slate" onClick={() => editCompany(company)}><Pencil size={15} /></button><button aria-label={`Eliminar ${company.name}`} className="grid h-9 w-9 place-items-center rounded-lg bg-red-50 text-red-600 hover:bg-red-100" disabled={deletingId === company.id} onClick={() => deleteCompany(company)}><Trash2 size={15} /></button></div></td></tr>)}{!filteredCompanies.length && <tr><td className="px-5 py-10 text-center text-late4-slate" colSpan={7}>Todavía no hay empresas registradas.</td></tr>}</tbody></table></div>
        ) : (
      ...
      {selectedCompany && <div className="fixed inset-0 z-50 grid place-items-center bg-late4-ink/60 p-4 backdrop-blur-sm"><section className="max-h-[92vh] w-full max-w-3xl overflow-y-auto rounded-2xl bg-white shadow-2xl"><header className="flex items-start justify-between border-b border-late4-ink/10 bg-late4-paper px-6 py-6"><div><p className="eyebrow">Ficha individual de empresa</p><h2 className="mt-2 text-3xl font-extrabold">{selectedCompany.name}</h2><p className="mt-3 flex items-center gap-2 text-2xl font-extrabold text-late4-teal"><MapPin size={22} />{selectedCompany.location || 'Ciudad sin definir'}</p></div><button aria-label="Cerrar ficha" className="grid h-10 w-10 place-items-center rounded-lg bg-white" onClick={() => setSelectedCompany(null)}><X size={18} /></button></header><div className="p-6"><div className="grid gap-4 sm:grid-cols-2"><Detail label="Actividad" value={selectedCompany.industry} /><Detail label="Tamaño / señal" value={selectedCompany.companySize} /><Detail label="Potencial ISO" value={selectedCompany.isoPotential === 'very_high' ? 'Muy alto' : selectedCompany.isoPotential === 'high' ? 'Alto' : undefined} /><Detail label="Estado comercial" value={selectedCompany.status === 'active' ? 'Cliente' : selectedCompany.status === 'inactive' ? 'Inactiva' : 'Prospecto'} /><Detail label="Contacto" value={selectedCompany.publicContact || 'Pendiente de relevamiento'} /><Detail label="Grupo empresario" value={selectedCompany.groupName} /><Detail label="Parque / distrito" value={selectedCompany.parkName} /><Detail label="Notas" value={selectedCompany.notes} /></div><div className="mt-6 flex flex-wrap gap-3 border-t border-late4-ink/10 pt-5"><a className="btn-primary gap-2" href={mapsUrl(selectedCompany)} rel="noreferrer" target="_blank"><MapPinned size={17} /> Abrir en Google Maps</a>{selectedCompany.website && <a className="btn-secondary gap-2" href={selectedCompany.website} rel="noreferrer" target="_blank"><ExternalLink size={17} /> Sitio web</a>}<button className="btn-secondary gap-2" onClick={() => { setSelectedCompany(null); editCompany(selectedCompany); }}><Pencil size={17} /> Editar empresa</button></div></div></section></div>}
    </div>
  );
}
...
function Detail({ label, value }: { label: string; value?: string }) {
  return <div className="rounded-xl bg-late4-paper p-4"><p className="text-[11px] font-extrabold uppercase tracking-wider text-late4-slate">{label}</p><p className="mt-2 text-sm font-bold text-late4-ink">{value || 'Sin información'}</p></div>;
}
